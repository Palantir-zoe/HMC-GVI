...
from data import load_german_credit_dataset, load_pima_dataset
from mcmc import (
    adaptive_mh,
    hmc,
    mala,
...
from targets import logistic_grad_logposterior, logistic_logposterior
...
RETUNED_SETTINGS = {
    ...
    "hmc_tune_steps": 160,
    "hmc_eval_steps": 400,
    "hmc_leapfrog_candidates": (4, 6, 8, 10, 12, 16, 20, 24, 32, 40),
}
...
def _run_one_dataset(name: str, y: np.ndarray, x: np.ndarray, cfg: dict, seed: int) -> list[dict]:
    ...
    hmc_main_tune_time = now() - hmc_main_tune_start
    del hmc_burn
    gc.collect()
    hmc_main_start = now()
    hmc_run = hmc(
...
def main() -> None:
    ...
    parser.add_argument(
        "--output",
        type=str,
        default=str(Path(__file__).resolve().parents[1] / "results" / "logistic_pima_german.csv"),
    )
